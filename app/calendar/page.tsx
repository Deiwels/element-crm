                        ...
                        <div key={ev.id} className={`cal-event${isArrived ? ' arrived-pulse' : ''}${isPaid ? ' cal-event-paid' : ''}${drag?.eventId===ev.id ? ' cal-event-dragging' : ''}`}
                          ...
                          {tinyCol ? (<>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                              ...
                              {(() => { const cs = ev._raw?.client_status; const m: Record<string,{l:string,c:string}> = { vip:{l:'V',c:'#ffd700'}, active:{l:'A',c:'#8ff0b1'}, new:{l:'N',c:'#7abaff'}, at_risk:{l:'!',c:'#ff6b6b'}, risk:{l:'!',c:'#ff6b6b'} }; const b = m[cs]; return b ? <span style={{ fontSize: 6, fontWeight: 900, color: b.c, flexShrink: 0 }}>{b.l}</span> : null })()}
                            </div>
                            ...
                            {height > 48 && <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                              <span style={{ fontSize: 7, color: ev.paid ? 'rgba(143,240,177,.70)' : 'rgba(255,255,255,.30)' }}>{ev.paid ? '✓' : ev.status?.charAt(0).toUpperCase()}</span>
