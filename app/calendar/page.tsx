...
const DAY_DEFAULTS = [
  ...
  { enabled: true,  startMin: 10*60, endMin: 20*60 }, // Sat
]
...
function DatePickerModal({ current, onSelect, onClose }: {
  ...
  function goToday()   { const m = new Date(); m.setDate(1); m.setHours(0,0,0,0); setMonth(m) }
  ...
  const btnBase: React.CSSProperties = { height: 44, borderRadius: 14, border: '1px solid rgba(255,255,255,.12)', background: 'rgba(0,0,0,.18)', color: '#fff', cursor: 'pointer', fontWeight: 900, fontSize: 14, fontFamily: 'inherit', transition: 'all .18s ease' }

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 18 }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={{ width: 'min(480px, 95vw)', borderRadius: 20, border: '1px solid rgba(255,255,255,.12)', background: 'linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.03))', backdropFilter: 'blur(18px)', padding: 16, color: '#e9e9e9', fontFamily: 'Inter, sans-serif' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <button onClick={goToday} style={{ height: 36, padding: '0 14px', borderRadius: 999, border: '1px solid rgba(255,255,255,.12)', background: 'rgba(255,255,255,.05)', color: '#fff', cursor: 'pointer', fontWeight: 900, fontSize: 12, fontFamily: 'inherit' }}>Today</button>
        </div>
          ...
          {days.map((d, i) => {
            ...
            return (
              <button key={i} onClick={() => { onSelect(d); onClose() }}
                style={{ ...btnBase, opacity: inMonth ? 1 : 0.35, borderColor: isSel ? 'rgba(10,132,255,.75)' : isToday ? 'rgba(255,207,63,.55)' : 'rgba(255,255,255,.12)', background: isSel ? 'rgba(10,132,255,.12)' : 'rgba(0,0,0,.18)', boxShadow: isSel ? '0 0 0 1px rgba(10,132,255,.22) inset, 0 0 18px rgba(10,132,255,.18)' : 'none' }}>
                {d.getDate()}
              </button>
            )
          })}
        ...
        <div style={{ marginTop: 12, fontSize: 12, color: 'rgba(255,255,255,.40)', textAlign: 'center' }}>Click any day to open it in the calendar</div>
      </div>
    </div>
  )
}
...
// ─── SCHEDULE GRID ────────────────────────────────────────────────────────────
function SchedGrid({ schedule, onChange }: {
  ...
  function setTime(i: number, field: 'startMin' | 'endMin', val: string) {
    const next = [...schedule]
    next[i] = { ...next[i], [field]: timeStrToMin(val) }
    onChange(next)
  }
      ...
      {DAY_NAMES.map((name, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 4, border: `1px solid ${day.enabled ? 'rgba(10,132,255,.55)' : 'rgba(255,255,255,.10)'}`, borderRadius: 12, padding: '8px 6px', background: day.enabled ? 'rgba(10,132,255,.08)' : 'rgba(0,0,0,.18)', opacity: day.enabled ? 1 : 0.55, transition: 'all .18s' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 3, opacity: day.enabled ? 1 : 0.3, pointerEvents: day.enabled ? 'auto' : 'none' }}>
              <label style={{ fontSize: 9, letterSpacing: '.10em', textTransform: 'uppercase', color: 'rgba(255,255,255,.40)' }}>From</label>
              <input type="time" value={minToTimeStr(day.startMin)} onChange={e => setTime(i, 'startMin', e.target.value)}
                style={{ height: 30, borderRadius: 10, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.22)', color: '#fff', padding: '0 6px', fontSize: 11, outline: 'none', width: '100%', colorScheme: 'dark' as any }} />
              <label style={{ fontSize: 9, letterSpacing: '.10em', textTransform: 'uppercase', color: 'rgba(255,255,255,.40)' }}>To</label>
              <input type="time" value={minToTimeStr(day.endMin)} onChange={e => setTime(i, 'endMin', e.target.value)}
                style={{ height: 30, borderRadius: 10, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.22)', color: '#fff', padding: '0 6px', fontSize: 11, outline: 'none', width: '100%', colorScheme: 'dark' as any }} />
            </div>
          </div>
        )
...
function SettingsModal({
  ...
  function scheduleToPayload(sched: DaySchedule[]) {
  ...
  function initEditSchedule(b: Barber) {
    if (editSchedules[b.id]) return
    // Try to restore from barber.schedule.perDay if available
    const raw = (b as any)._raw
    const perDay = raw?.schedule?.perDay || raw?.work_schedule?.perDay
    if (Array.isArray(perDay) && perDay.length === 7) {
      setEditSchedules(prev => ({ ...prev, [b.id]: perDay.map((d: any) => ({ enabled: !!d.enabled, startMin: Number(d.startMin)||10*60, endMin: Number(d.endMin)||20*60 })) }))
    } else {
      setEditSchedules(prev => ({ ...prev, [b.id]: DAY_DEFAULTS.map(d => ({...d})) }))
    }
  }
  ...
  async function addBarber() {
    ...
    try {
      const schedPayload = scheduleToPayload(bSchedule)
      await apiFetch('/api/barbers', {
        method: 'POST',
        body: JSON.stringify({
          name: bName.trim(), level: bLevel.trim(), about: bAbout.trim(),
          ...
          active: true, public_enabled: true,
  ...
  async function saveBarberEdit(b: Barber, editLevel: string, editPrice: string, editPhotoFile: File | null) {
    const sched = editSchedules[b.id] || DAY_DEFAULTS.map(d => ({...d}))
    const schedPayload = scheduleToPayload(sched)
    let photoUrl = b.photo || ''
    if (editPhotoFile) photoUrl = await compressPhoto(editPhotoFile)
    await apiFetch(`/api/barbers/${encodeURIComponent(b.id)}`, {
      method: 'PATCH',
      body: JSON.stringify({
        level: editLevel, base_price: editPrice, photo_url: photoUrl,
        schedule: schedPayload, work_schedule: schedPayload,
  ...
  async function deleteBarber(id: string, name: string) {
    ...
    try {
      await apiFetch(`/api/barbers/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify({ active: false, public_enabled: false }) })
  ...
  async function addService() {
    ...
    try {
      ...
      if (existing) {
        const ids = new Set(existing.barberIds); if (sBarber) ids.add(sBarber)
  ...
  const labelStyle: React.CSSProperties = { fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.55)', marginBottom: 6, display: 'block' }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 90, padding: 18, overflowY: 'auto' }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={{ width: 'min(960px, 96vw)', borderRadius: 20, border: '1px solid rgba(255,255,255,.12)', background: 'linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.03))', backdropFilter: 'blur(18px)', padding: 16, maxHeight: 'calc(100vh - 48px)', overflowY: 'auto', color: '#e9e9e9', fontFamily: 'Inter, sans-serif' }}>

        <div style={{ fontSize: 11, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,.60)', marginBottom: 4 }}>Settings</div>
        <div style={{ fontSize: 12, color: 'rgba(255,255,255,.45)', marginBottom: 14 }}>Barbers · Services · Clients</div>
        ...
        {tab === 'barbers' && (
          <div>
            {/* Add barber form */}
            <div style={{ padding: 14, borderRadius: 16, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.14)', marginBottom: 16 }}>
              <div style={{ fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,.50)', marginBottom: 12 }}>Add new barber</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
                {[
                  { label: 'Barber name *', val: bName, set: setBName, ph: 'Nazar' },
                  { label: 'Level', val: bLevel, set: setBLevel, ph: 'Senior / Expert / Ambassador' },
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <input value={bAbout} onChange={e => setBAbout(e.target.value)} placeholder="Strong detail work, specializes in…" style={inputStyle} />
                </div>
              </div>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  {bPhotoPreview && <img src={bPhotoPreview} alt="preview" style={{ width: 44, height: 44, borderRadius: 12, objectFit: 'cover', border: '1px solid rgba(255,255,255,.14)' }} />}
                  {bPhotoPreview && <button onClick={() => { setBPhotoFile(null); setBPhotoPreview('') }} style={{ ...dangerBtnStyle, height: 32, padding: '0 10px', fontSize: 11 }}>✕</button>}
                </div>
              </div>

              {/* Schedule grid */}
              <div style={{ marginBottom: 12 }}>
                <label style={labelStyle}>Working schedule — toggle days ON/OFF, set hours per day</label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {barbers.map(b => {
                const isEditing = editBarberId === b.id
                if (isEditing) { initEditSchedule(b) }
                const sched = editSchedules[b.id] || DAY_DEFAULTS.map(d => ({...d}))

                return (
                  <div key={b.id} style={{ borderRadius: 16, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.14)', padding: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: isEditing ? 14 : 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        {b.photo
                          ? <img src={b.photo} alt={b.name} style={{ width: 44, height: 44, borderRadius: 12, objectFit: 'cover', border: '1px solid rgba(255,255,255,.14)', flexShrink: 0 }} onError={e => (e.currentTarget.style.display='none')} />
                          : <div style={{ width: 44, height: 44, borderRadius: 12, background: b.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 900, flexShrink: 0 }}>{b.name[0]}</div>
                        }
                        <div>
                          <div style={{ fontWeight: 900, fontSize: 14 }}>{b.name}</div>
                          <div style={{ fontSize: 11, color: 'rgba(255,255,255,.45)', letterSpacing: '.08em', textTransform: 'uppercase' }}>{b.level || 'Barber'}{b.photo ? '' : ''}</div>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => { setEditBarberId(isEditing ? null : b.id); if (!isEditing) initEditSchedule(b) }} style={{ ...btnStyle, borderColor: isEditing ? 'rgba(10,132,255,.55)' : undefined, background: isEditing ? 'rgba(10,132,255,.12)' : undefined, color: isEditing ? '#d7ecff' : undefined }}>
                    ...
                    {isEditing && <BarberEditForm b={b} sched={sched} onSchedChange={s => setEditSchedules(prev => ({...prev, [b.id]: s}))} onSave={saveBarberEdit} inputStyle={inputStyle} labelStyle={labelStyle} primaryBtnStyle={primaryBtnStyle} dangerBtnStyle={dangerBtnStyle} />}
                  </div>
                )
              })}
            </div>
        ...
        {tab === 'services' && (
          <div>
            <div style={{ padding: 14, borderRadius: 16, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.14)', marginBottom: 16 }}>
              <div style={{ fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,.50)', marginBottom: 12 }}>Add / assign service</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
                <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {services.map(s => {
                ...
                return (
                  <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderRadius: 14, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.18)', gap: 12 }}>
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'rgba(255,255,255,.45)', letterSpacing: '.08em', textTransform: 'uppercase', marginTop: 2 }}>
                        {s.durationMin}min{s.price ? ` · $${s.price}` : ''} · {assigned.length ? assigned.join(', ') : 'All barbers'}
                      </div>
                    </div>
        ...
        {tab === 'clients' && (
          <div>
            <div style={{ padding: 14, borderRadius: 16, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.14)', marginBottom: 16 }}>
              <div style={{ fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,.50)', marginBottom: 12 }}>Client database (local)</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
                <div style={{ gridColumn: '1 / -1' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 320, overflowY: 'auto' }}>
              ...
              {clients.slice(0, 100).map(c => (
                <div key={c.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 14, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.18)', gap: 12 }}>
                  ...
                  <button onClick={() => { const list = clients.filter((x: any) => x.id !== c.id); saveClients(list) }} style={dangerBtnStyle}>Delete</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'account' && (
          <div>
            ...
            <button onClick={() => { localStorage.removeItem('ELEMENT_TOKEN'); localStorage.removeItem('ELEMENT_USER'); window.location.href = '/signin' }} style={{ ...dangerBtnStyle, height: 44, width: '100%' }}>Log out</button>
          </div>
        )}

        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16, paddingTop: 12, borderTop: '1px solid rgba(255,255,255,.08)' }}>
...
// ─── BARBER EDIT FORM (sub-component to avoid hooks-in-loop issue) ────────────
function BarberEditForm({ b, sched, onSchedChange, onSave, inputStyle, labelStyle, primaryBtnStyle, dangerBtnStyle }: {
  b: Barber; sched: DaySchedule[]
  onSchedChange: (s: DaySchedule[]) => void
  onSave: (b: Barber, level: string, price: string, photo: File | null) => void
  inputStyle: React.CSSProperties; labelStyle: React.CSSProperties
  primaryBtnStyle: React.CSSProperties; dangerBtnStyle: React.CSSProperties
}) {
  const [level, setLevel] = useState(b.level || '')
  const [price, setPrice] = useState((b as any).basePrice || '')
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          <label style={labelStyle}>Change photo</label>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <label style={{ height: 40, padding: '0 14px', borderRadius: 12, border: '1px solid rgba(255,255,255,.12)', background: 'rgba(255,255,255,.05)', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', fontSize: 12, fontFamily: 'inherit' }}>
            ...
            {b.photo && !photoFile && <img src={b.photo} alt="" style={{ width: 40, height: 40, borderRadius: 10, objectFit: 'cover', border: '1px solid rgba(255,255,255,.12)' }} onError={e => (e.currentTarget.style.display='none')} />}
          </div>
        </div>
      </div>
      <div style={{ marginBottom: 12 }}>
        <label style={labelStyle}>Working schedule</label>
        <SchedGrid schedule={sched} onChange={onSchedChange} />
      </div>
      <button onClick={async () => { setSaving(true); await onSave(b, level, price, photoFile); setSaving(false) }} disabled={saving}
        style={{ ...primaryBtnStyle, width: '100%', height: 42 }}>
