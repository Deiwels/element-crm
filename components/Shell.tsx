...
interface User { uid: string; name: string; username: string; role: string; barber_id?: string; photo?: string }
...
// ─── Profile Modal ────────────────────────────────────────────────────────────
function ProfileModal({ user, onClose, onUpdated }: { user: User; onClose: () => void; onUpdated: (u: User) => void }) {
  ...
  async function saveProfile() {
    ...
    try {
      // Update barber photo + name if barber role
      if (user.barber_id) {
        const token = localStorage.getItem('ELEMENT_TOKEN') || ''
        await fetch(`${API}/api/barbers/${encodeURIComponent(user.barber_id)}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, 'X-API-KEY': API_KEY },
          body: JSON.stringify({ name, photo_url: photo })
        })
      }
      // Update user name
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, backdropFilter: 'blur(10px)' }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={{ width: 'min(440px,95vw)', borderRadius: 20, border: '1px solid rgba(255,255,255,.12)', background: 'linear-gradient(180deg,rgba(20,20,30,.92),rgba(10,10,20,.90))', backdropFilter: 'blur(24px)', color: '#e9e9e9', fontFamily: 'Inter,sans-serif', boxShadow: '0 24px 80px rgba(0,0,0,.7)', overflow: 'hidden' }}>
        {/* Header */}
        ...
        <div style={{ display: 'flex', gap: 6, padding: '12px 18px 0' }}>
          {(['profile', 'password'] as const).map(t => (
            <button key={t} onClick={() => { setTab(t); setMsg(''); setErr('') }}
              style={{ height: 32, padding: '0 14px', borderRadius: 999, border: `1px solid ${tab===t ? 'rgba(10,132,255,.55)' : 'rgba(255,255,255,.10)'}`, background: tab===t ? 'rgba(10,132,255,.14)' : 'rgba(255,255,255,.04)', color: tab===t ? '#d7ecff' : 'rgba(255,255,255,.60)', cursor: 'pointer', fontWeight: 900, fontSize: 11, textTransform: 'uppercase', letterSpacing: '.08em', fontFamily: 'inherit' }}>
        ...
        <div style={{ padding: '16px 18px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          {tab === 'profile' && <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {photo && <button onClick={() => setPhoto('')} style={{ marginTop: 6, height: 28, padding: '0 10px', borderRadius: 8, border: '1px solid rgba(255,107,107,.30)', background: 'rgba(255,107,107,.06)', color: '#ffd0d0', cursor: 'pointer', fontSize: 11, fontFamily: 'inherit' }}>Remove photo</button>}
              </div>
            </div>
            ...
            <div>
              ...
              <input value={user.username || ''} disabled style={{ ...inp, opacity: .45, cursor: 'not-allowed' }} />
            </div>

            <div>
              <label style={lbl}>Role</label>
              <input value={user.role || ''} disabled style={{ ...inp, opacity: .45, cursor: 'not-allowed', textTransform: 'capitalize' }} />
            </div>

            <button onClick={saveProfile} disabled={saving}
              style={{ height: 44, borderRadius: 12, border: '1px solid rgba(10,132,255,.65)', background: 'rgba(10,132,255,.14)', color: '#d7ecff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit' }}>
          ...
          {msg && <div style={{ fontSize: 12, color: '#c9ffe1', padding: '8px 12px', borderRadius: 10, border: '1px solid rgba(143,240,177,.30)', background: 'rgba(143,240,177,.08)' }}>{msg}</div>}
...
export default function Shell({ children, page }: { children: React.ReactNode; page: string }) {
  ...
  if (status === 'loading' || status === 'noauth') return <div style={{ background: '#000', minHeight: '100vh' }} />
      ...
      <div className="shell">
        <aside className="sidebar">
          ...
          <div className="user-bar">
            <div style={{ padding: '10px 12px', borderRadius: 12, background: 'rgba(0,0,0,.20)', border: '1px solid rgba(255,255,255,.06)' }}>
              {/* Avatar + name row */}
              <button onClick={() => setShowProfile(true)}
                ...
                {(user as any)?.photo
                  ? <img src={(user as any).photo} alt={user?.name||''} style={{ width: 36, height: 36, borderRadius: 10, objectFit: 'cover', border: '1px solid rgba(255,255,255,.14)', flexShrink: 0 }} onError={e => (e.currentTarget.style.display='none')} />
                  : <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(10,132,255,.20)', border: '1px solid rgba(10,132,255,.30)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 900, color: '#d7ecff', flexShrink: 0 }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  <div style={{ fontSize: 10, letterSpacing: '.08em', textTransform: 'uppercase', color: 'rgba(255,255,255,.35)', marginTop: 2 }}>{user?.role || '—'}</div>
                </div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,.30)', flexShrink: 0 }}>✎</div>
              </button>

              <button onClick={() => { localStorage.removeItem('ELEMENT_TOKEN'); localStorage.removeItem('ELEMENT_USER'); window.location.href = '/signin' }}
                style={{ height: 30, width: '100%', borderRadius: 8, cursor: 'pointer', fontSize: 11, fontWeight: 700, border: '1px solid rgba(255,107,107,.30)', background: 'rgba(255,107,107,.06)', color: '#ffd0d0', fontFamily: 'inherit' }}>
                Log out
              </button>
            </div>
          </div>
